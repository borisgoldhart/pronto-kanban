...
const ICON_CHILD = '<svg viewBox="0 0 16 16" width="11" height="11" aria-hidden="true"><path fill="currentColor" d="M3 2h1.6v7.2h6.1L8.9 7.4l1.1-1.1 3.6 3.6-3.6 3.6-1.1-1.1 1.8-1.8H3V2Z"/></svg>';
...
export function cardPreview(task: BoardTask, statusName: string, statusColor: string): string {
  ...
  const dates = [task.startDate, task.endDate].filter(Boolean).map((d) => enc(d)).join(" to ");
  return `<div class="pk-tip">
    ...
    ${row("Assigned", people)}
    ${row("Dates", dates)}
    ${task.parentId ? row("Parent task", `#${enc(task.parentId)}`) : ""}
